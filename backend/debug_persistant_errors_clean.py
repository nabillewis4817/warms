...
def main():
    print("DEMARRAGE DU DIAGNOSTIC DES ERREURS PERSISTANTES")
    ...
    for method, url, status, response in results:
        if status and status >= 400:
            ...
            print(f"ERREUR {status} {method} {url}")
    
    print(f"\nStatistiques des erreurs:")
    for error_type, count in error_counts.items():
        print(f"   {error_type}: {count} occurrence(s)")
    
    # Tests specifiques pour diagnostiquer
    print(f"\nTESTS DE DIAGNOSTIC SPECIFIQUES:")
    
    # Test 1: Verifier le modele Message
    try:
        from messagerie.models import Message
        messages_count = Message.objects.count()
        print(f"Modele Message: {messages_count} message(s) trouve(s)")
        ...
        print(f"Erreur modele Message: {e}")
    
    # Test 2: Verifier le service OCR
    try:
        from assistant_ia.services_ocr import extraire_texte_image
        from io import BytesIO
        
        # Creer une fausse image pour tester
    ...
    # Test 3: Verifier les roles utilisateurs
    try:
        patients = Utilisateur.objects.filter(role__in=['patient', 'PATIENT'])
        print(f"Utilisateurs patients: {patients.count()} trouve(s)")
        
        for patient in patients[:3]:  # Limiter a 3 pour l'affichage
            print(f"   - {patient.username} (role: '{patient.role}')")
    except Exception as e:
        print(f"Erreur verification roles: {e}")
